// ------------------------------------------------
// epollserver4.cpp
// server4 epoll ET 이벤트 루프 서버 구현부
//
// 핵심 원칙:
//   1. epoll ET 모드: 이벤트 발생 시 EAGAIN까지 루프로 읽어야 함
//   2. 모든 소켓은 non-blocking
//   3. epoll 스레드가 router4 호출 → dispatcher 거쳐 처리
// ------------------------------------------------
...
// 전역 router4 인스턴스
static router4 g_router4;

// ------------------------------------------------
// 생성자
// ------------------------------------------------
epollserver4::epollserver4(int port)
    : port_(port)
    , listenfd_(-1)
    , epollfd_(-1)
    , running_(false)
...
// ------------------------------------------------
void epollserver4::run()
{
    if (!init_listen_socket())
        throw std::runtime_error("epollserver4: 리슨 소켓 초기화 실패");

    if (!init_epoll())
        throw std::runtime_error("epollserver4: epoll 초기화 실패");

    // 리슨 소켓을 epoll ET로 등록
    if (!add_to_epoll(listenfd_, EPOLLIN | EPOLLET))
        throw std::runtime_error("epollserver4: listenfd epoll 등록 실패");

    running_.store(true);

    log_event("epollserver4",
              "서버 시작 | port=" + std::to_string(port_) +
              " | 모드=epoll ET non-blocking");

    epoll_event events[EPOLL_MAX_EVENTS4];
    ...
    while (running_.load())
    {
        ...
        if (nfds < 0)
        {
            ...
            log_event("epollserver4", "epoll_wait 오류 | 루프 종료");
            break;
        }
...
// 이벤트 루프 종료 요청
// ------------------------------------------------
void epollserver4::stop()
{
    running_.store(false);
    log_event("epollserver4", "stop 요청");
}
...
// non-blocking 설정
// ------------------------------------------------
bool epollserver4::init_listen_socket()
{
    ...
    {
        log_event("epollserver4", "listen 실패");
        return false;
    }
...
// epoll 인스턴스 생성
// ------------------------------------------------
bool epollserver4::init_epoll()
{
    epollfd_ = epoll_create1(0);
    if (epollfd_ < 0)
    {
        log_event("epollserver4", "epoll_create1 실패");
        return false;
    }

    log_event("epollserver4", "epoll 인스턴스 생성 완료");
    return true;
}
...
// ------------------------------------------------
bool epollserver4::add_to_epoll(int fd, uint32_t events)
{
    epoll_event ev{};
    ev.events  = events;
    ev.data.fd = fd;
...
// fd를 epoll에서 제거
// ------------------------------------------------
void epollserver4::remove_from_epoll(int fd)
{
...
// fd를 O_NONBLOCK으로 설정
// ------------------------------------------------
bool epollserver4::set_nonblocking(int fd)
{
...
// ------------------------------------------------
void epollserver4::handle_accept()
{
    while (true)
    {
        ...
        if (clientfd < 0)
        {
            ...
            log_event("epollserver4", "accept 오류");
            break;
        }
...
// ------------------------------------------------
void epollserver4::handle_read(int fd)
{
    char buf[4096];
...
// ------------------------------------------------
void epollserver4::handle_close(int fd)
{
    log_event("epollserver4",
...
// ------------------------------------------------
void epollserver4::process_buffer(int fd)
{
    ...
    while (true)
    {
        ...
        log_event("epollserver4",
                  ...
                  " | bodylen=" + std::to_string(bodylen));
        log_flow("?", "epollserver4", "router4",
                 "fd=" + std::to_string(fd));

        // router4로 전달 (dispatcher 거쳐 처리 경로 결정)
        g_router4.route(fd, jsonbody);
    }
}
