...
// ------------------------------------------------
// eventbus4.h
// server4 pub/sub 이벤트 버스
//
// 역할:
//   - Service가 EventBus를 통해 응답 publish
//   - main.cpp의 "response" 구독자가 fd로 write
//   - 모든 publish/subscribe 멀티스레드 안전
//
// 구조:
//   publisher → eventbus4.publish(topic, data)
...
public:
    ...
    // instance
    // 싱글턴 인스턴스 반환
    // server4 전역에서 단일 eventbus4를 공유한다
    // ------------------------------------------------
    static eventbus4& instance();
...
private:
    ...
    // 복사/이동 금지
    eventbus4(const eventbus4&)            = delete;
    eventbus4& operator=(const eventbus4&) = delete;
