...
// ------------------------------------------------
// eventbus3.h
// server3 내부 pub/sub 이벤트 버스
// 모든 요청/응답이 EventBus를 경유한다
//
// 구조:
//   router → eventbus3.publish("request", payload)
//          → handler subscribe("request")
//          → handler 처리 후 publish("response", payload)
...
public:
    ...
    // instance
    // 싱글턴 인스턴스 반환
    // 서버 전역에서 단일 eventbus3를 공유한다
    // ------------------------------------------------
    static eventbus3& instance();
...
private:
    ...
    // 복사/이동 금지
    eventbus3(const eventbus3&)            = delete;
    eventbus3& operator=(const eventbus3&) = delete;
