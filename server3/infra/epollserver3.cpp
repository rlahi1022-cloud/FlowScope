// ------------------------------------------------
// epollserver3.cpp
// server3 epoll ET 이벤트 루프 서버 구현부
//
// 핵심 원칙:
...
// 생성자
// ------------------------------------------------
epollserver3::epollserver3(int port)
    : port_(port)
    , listenfd_(-1)
    , epollfd_(-1)
    , running_(false)
...
// ------------------------------------------------
void epollserver3::run()
{
    if (!init_listen_socket())
        throw std::runtime_error("epollserver3: 리슨 소켓 초기화 실패");

    if (!init_epoll())
        throw std::runtime_error("epollserver3: epoll 초기화 실패");

    // 리슨 소켓을 epoll ET로 등록
    if (!add_to_epoll(listenfd_, EPOLLIN | EPOLLET))
        throw std::runtime_error("epollserver3: listenfd epoll 등록 실패");

    running_.store(true);

    log_event("epollserver3",
              "서버 시작 | port=" + std::to_string(port_) +
              " | 모드=epoll ET non-blocking | EventBus pub/sub");

    epoll_event events[EPOLL_MAX_EVENTS3];
    ...
    while (running_.load())
    {
        ...
        if (nfds < 0)
        {
            ...
            log_event("epollserver3", "epoll_wait 오류 | 루프 종료");
            break;
        }
...
// 이벤트 루프 종료 요청
// ------------------------------------------------
void epollserver3::stop()
{
    running_.store(false);
    log_event("epollserver3", "stop 요청");
}
...
// non-blocking 설정
// ------------------------------------------------
bool epollserver3::init_listen_socket()
{
    ...
    {
        log_event("epollserver3", "listen 실패");
        return false;
    }
...
// epoll 인스턴스 생성
// ------------------------------------------------
bool epollserver3::init_epoll()
{
    epollfd_ = epoll_create1(0);
    if (epollfd_ < 0)
    {
        log_event("epollserver3", "epoll_create1 실패");
        return false;
    }

    log_event("epollserver3", "epoll 인스턴스 생성 완료");
    return true;
}
...
// ------------------------------------------------
bool epollserver3::add_to_epoll(int fd, uint32_t events)
{
    epoll_event ev{};
    ev.events  = events;
    ev.data.fd = fd;
...
// fd를 epoll에서 제거
// ------------------------------------------------
void epollserver3::remove_from_epoll(int fd)
{
...
// fd를 O_NONBLOCK으로 설정
// ------------------------------------------------
bool epollserver3::set_nonblocking(int fd)
{
...
// ------------------------------------------------
void epollserver3::handle_accept()
{
    while (true)
    {
        ...
        if (clientfd < 0)
        {
            ...
            log_event("epollserver3", "accept 오류");
            break;
        }
...
// ------------------------------------------------
void epollserver3::handle_read(int fd)
{
    char buf[4096];
...
// ------------------------------------------------
void epollserver3::handle_close(int fd)
{
    log_event("epollserver3",
...
// ------------------------------------------------
void epollserver3::process_buffer(int fd)
{
    ...
    while (true)
    {
        ...
        log_event("epollserver3",
                  ...
                  " | bodylen=" + std::to_string(bodylen));
        log_flow("?", "epollserver3", "router3",
                 "fd=" + std::to_string(fd));

        // router3로 전달 (traceid는 router3에서 발급)
