...
// 생성자
// ------------------------------------------------
jobqueue::jobqueue(std::size_t maxsize)
    : maxsize_(maxsize)
    , shutdown_(false)
{
    log_event("JobQueue", "초기화 완료", "최대 크기: " + std::to_string(maxsize));
}
...
bool jobqueue::push(const job& j)
{
    ...
    if (shutdown_.load()) {
        log_event("JobQueue", "push 거부 - shutdown 상태", "fd: " + std::to_string(j.fd));
        return false;
    }

    // 큐가 최대 크기에 도달했으면 drop (backpressure)
    if (queue_.size() >= maxsize_) {
        log_event("JobQueue", "push 거부 - 큐 가득 참 (drop)", "fd: " + std::to_string(j.fd)
    ...
    log_event("JobQueue", "push 완료", "fd: " + std::to_string(j.fd)
...
bool jobqueue::pop(job& out)
{
    ...
    log_event("JobQueue", "pop 완료", "fd: " + std::to_string(out.fd)
...
{
    {
        ...
        log_event("JobQueue", "shutdown 요청 처리");
    }
    // 모든 대기 스레드 깨움
    cond_.notify_all();
}
